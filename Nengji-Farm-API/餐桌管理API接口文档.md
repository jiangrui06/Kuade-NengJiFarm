...
| 字段 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| scope | string | 否 | `list`=全部状态，`form`=排除"删除"，`toggle`=仅"使用中"+"停用"（不传则返回全部） |
...
| 接口 | 方法 | 路径 | 说明 |
|------|:----:|------|------|
| 获取状态列表 | GET | `/api/dining-table/statuses?scope=` | 支持 `?scope=form` 排除"删除"，`?scope=toggle` 仅"使用中"+"停用" |
...
| 新增/编辑页（table-form.html） | `scope=form` | 状态下拉（排除"删除"） |
| 状态切换按钮 | `scope=toggle` | 仅"使用中"↔"停用"（排除"删除"） |
