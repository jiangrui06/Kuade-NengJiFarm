...
Page({
    ...
    api.get('/api/categories/list')
      .then(categories => {
        ...
        this.setData({
          ...
          activeCategory: processedCategories.length > 0 ? processedCategories[0].id : 'vegetables',
          loading: false
        })
  ...
  addImageUrlsToGoods(goods, category) {
    return goods.map(item => ({
      ...
      id: item.id,
      name: item.name,
      image: this.processImageUrl(item.image_url),
      price: item.price,
  ...
  loadCategoryGoods(category, isLoadMore) {
    ...
    // 获取分类下的菜品
    api.get('/api/dishes/list', { 
      category_id: category,
      page: nextPage, 
      pageSize: this.data.pageSize 
    }).then(data => {
      const newGoods = this.addImageUrlsToGoods(data || [], category);
      const old = this.data.goodsList[category] || [];
      this.setData({
        [`goodsList.${category}`]: old.concat(newGoods),
        [`pageMap.${category}`]: nextPage,
        [`hasMoreMap.${category}`]: (data || []).length >= this.data.pageSize,
        lazyLoading: false
      });
      this.updateMergedGoodsList();
